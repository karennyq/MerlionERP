<%-- 
    Document   : updateSalesQuotation
    Created on : Sep 30, 2011, 1:26:17 PM
    Author     : Randy
--%>
...
<html>
    <head>
        ...
        <script type="text/javascript"> 
            <% session.setAttribute("updateQuotationLineItemList" + request.getParameter("pre_sale_doc_id"), new ArrayList()); %>
            <% session.setAttribute("updateQuotationDiscount" + request.getParameter("pre_sale_doc_id"), "0.00"); %>
                    ...
                    onAfterEdit:function(index,row){  
                        ...
                        $.ajax({
                            ...
                            data: "action=updateProductItem"
                                ...
                                + "&listIndex=" + index
                                + "&listName=" + "updateQuotationLineItemList<%= request.getParameter("pre_sale_doc_id")%>",
                            dataType: "json",
                            cache: false,
                            success: function(data){
                                if(data.type!='error'){
            ...
            function getSalesQuotationDetails(){
                $.ajax({
                    type: "POST",
                    url: "../SalesQuotationServlet",
                    data: "action=getSalesQuotationDetails"
                        + "&pre_sale_doc_id=" + $('#pre_sale_doc_id').val()
                        + "&listName=" + "updateQuotationLineItemList<%= request.getParameter("pre_sale_doc_id")%>"
                        + "&discountName=" + "updateQuotationDiscount<%= request.getParameter("pre_sale_doc_id")%>",
                    dataType: "json",
                    cache: false,
                    success: function(data){
            ...
            $(function(){
                $('#ff').form({
                    url:'../SalesQuotationServlet?action=updateQuotation&listName=updateQuotationLineItemList<%= request.getParameter("pre_sale_doc_id")%>&discountName=updateQuotationDiscount<%= request.getParameter("pre_sale_doc_id")%>',
                    onSubmit:function(){   
    ...
    <body>
        <div style="padding:3px 2px;border-bottom:1px solid #ccc"><h2>Update Sales Quotation</h2></div>  
        <br/>
        <form id="ff" method="post">
            ...
            <br/>
            <div id="updateQuotation" name="updateQuotation" class="easyui-panel" title="Quotation Details:" style="padding:0px;background:#fafafa;" collapsible="true"> 
                <table class="tableForms"> 
                    <tr>
            ...
            <div id="pdtDetails" name="pdtDetails" class="easyui-panel" title="Product Details:" style="padding:0px;background:#fafafa;" collapsible="true" >
                ...
                <div id="pdt-add-tt" name="pdt-add-tt" fit=true style="height:250px;">
                    <table id="pdttt" name="pdttt" class="easyui-datagrid" fit="true" style="width:100%; height:100px;" 
                           url="../SalesQuotationServlet?action=getProductList&listName=updateQuotationLineItemList<%= request.getParameter("pre_sale_doc_id")%>&discountName=updateQuotationDiscount<%= request.getParameter("pre_sale_doc_id")%>"
                           fitColumns="true"
                           rownumbers="true"
                           showfooter="true">  
                        <thead>  
