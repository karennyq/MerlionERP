...
public class CustomerFacade extends AbstractFacade<Customer> implements CustomerFacadeLocal {
    ...
    public Collection findFilteredCustomers(int page, int rows, String sort, String order, String inquirer_id, String company_name) {
        Query query = em.createQuery("SELECT c FROM Customer c WHERE c.inquirer_id LIKE ?1 AND c.company_name LIKE ?2 ORDER BY c." + sort + " " + order);
        query.setParameter(1, "%" + inquirer_id + "%");
        query.setParameter(2, "%" + company_name + "%");
